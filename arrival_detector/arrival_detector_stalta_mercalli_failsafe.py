...
def int_validator(int_value):
    try: 
        ...
        raise argparse.ArgumentTypeError(f"Argument '{int_value}' is unvalid. Must be an integer.")
def local_parser():
    ...
    parser.add_argument('--min_distance', type=float, default=20, required=False, help='minimum distance from POI to fault to consider for arrival algorithm')
...
#filter_amplitude: bool = True
    ...
    for file in files:
        ...
        for i, idx in enumerate(final_idx):
            ...
            # check for further detection errors
            # if the estimated arrival time is smaller than the arrival of the maximum amplitude
            if time[idx] <= time[max_elevation_idx]:
            ...
            # if the estimated arrival time is larger than the arrival of the maximum elevation
            # then it means that the maximum amplitude happens before than the first arrival...
            # do not crop
            else:
                cropped_time_series[valid_ids[i]] = valid_eta[:, i]
                # save crop time for each poi time series, no crop, then crop time is zero
                cropped_time_series[f'crop_time_{valid_ids[i]}'] = 0
        # save valid ids
        cropped_time_series['valid_ids'] = valid_ids
        # save to pickle
        pklfilename = f"{file.split('/')[-1].split('.')[0]}_exp_failsafe.pkl"
