...
def int_validator(int_value):
    try:
        return int(int_value)
    except ValueError:
        raise argparse.ArgumentTypeError(f"Argument '{int_value}' is invalid. Must be an integer.")
...
def local_parser():
    ...
    parser.add_argument('--deltat',type=float, default=30, required=False, help='time interval in seconds between elevation computation')
    parser.add_argument('--sta', type=int_validator, default=20, required=False, help='define the length of the STA window')
    parser.add_argument('--lta', type=int_validator, default=600, required=False, help='define he length of the LTA window')
    parser.add_argument('--trigger_value', type=float, default=8, required=False, help='Threshold for detecting an event')
...
def main():
    ...
    coast_value = local_opts.coast_value
    save_velocity = local_opts.save_velocity
    ...
    for file in files:
        ...
        if save_velocity:
            time, lon, lat, eta, ux, uy, \
                _, _, _, mask_eta, _, _, fault_info = hio.read_outputs(file)
        else:
            time, lon, lat, eta, _, _, \
                _, _, _, mask_eta, _, _, fault_info = hio.read_outputs(file)
        ...
        # Regardless of save_velocity, we always need to get valid_eta
        # ----------------------------------------------------------------
    ...
    if filter_offset:
        ...
        # metadata value for velocity
        cropped_time_series['has_velocity'] = save_velocity
        ...
        for i, idx in enumerate(final_idx):
            # save cropped time series (only if save_velocity is False)
            # elevation time series after cropping
            if not save_velocity:
                cropped_time_series[valid_ids[i]] = du.crop_time_series(valid_eta[:, i],
                                                                        idx)
            # velocities time series after cropping (only if save_velocity is True)
            if save_velocity:
                cropped_time_series[f'ux_{valid_ids[i]}'] = du.crop_time_series(valid_ux[:, i], idx)
                cropped_time_series[f'uy_{valid_ids[i]}'] = du.crop_time_series(valid_uy[:, i], idx)
        ...
        # save to pickle
        # name varies according to value of save_velocity
        if save_velocity:
            pklfilename = f"{file.split('/')[-1].split('.')[0]}_new_th_velocity.pkl"
        else:
            pklfilename = f"{file.split('/')[-1].split('.')[0]}_new_th.pkl"
